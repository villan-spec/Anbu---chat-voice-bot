...
export const generateResponse = async (
  ...
  try {
    ...
    const systemInstruction = language === Language.TAMIL 
    ...
    if (error instanceof Error) {
        if (language === Language.TAMIL) {
            return `மன்னிக்கவும், ஒரு பிழை ஏற்பட்டது: ${error.message}`;
        }
...
// New function to generate speech from text
export const generateSpeech = async (
  ...
  try {
    ...
    // Use a cheerful prefix for the TTS prompt as per Gemini documentation recommendations
